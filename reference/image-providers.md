...
1. **Your own image tool.** If you (the agent running this skill) already have a native text-to-image tool — an `image_gen` / image-generation tool or MCP — use it directly and save the result to `raw/<style>.png`. This is the case when Codex itself runs the skill, or when the host agent exposes image generation. No subprocess needed.
2. **Codex CLI.** Else, if Codex is available — the bundled `codex` skill (`~/.claude/skills/codex/scripts/ask_codex.sh`) or `codex` on PATH — delegate generation to it: pass the composed prompt and have it save the PNG to `raw/<style>.png`.
3. **External API.** Else, ask the user which image API to use and where the key lives. For **OpenAI Images (gpt-image)**, `scripts/gen_image.py --prompt "…" --out raw/<style>.png` is ready (reads `OPENAI_API_KEY`). For any other API (Replicate, Fal, Stability, a self-hosted model…), call it directly to produce the PNG.
