...
| 配置名 | 业务环境变量 | 系统凭据引用 |
| --- | --- | --- |
| default | `OPENAI_API_KEY` | `oil-icon/openai/default` |
...
```bash
node "$SKILL_DIR/scripts/credential-ui/src/profile.ts" run default -- python3 "$SKILL_DIR/scripts/gen_image.py" --prompt "图标设计要求" --out "/path/to/icons.png"
```
