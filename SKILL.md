...
4. **Generate the sheet(s).** Choose an image provider by capability, in order (see `reference/image-providers.md`) — never assume Codex is present:
   1. **Your own image tool** — if you already have a native text-to-image tool (e.g. Codex itself running this skill, or a host image-gen tool), call it directly.
   2. **Codex CLI** — else if Codex is available (`~/.claude/skills/codex/scripts/ask_codex.sh` or `codex` on PATH), delegate generation to it.
   3. **External API** — else ask the user which image API + key to use; for OpenAI Images, `scripts/gen_image.py --prompt "…" --out raw/<style>.png` is ready.
   Save each sheet to `raw/<style>.png` on flat grey `#808080`. One sheet = one generation = one consistent style; a provider only has to turn the prompt into a grey-background PNG.
